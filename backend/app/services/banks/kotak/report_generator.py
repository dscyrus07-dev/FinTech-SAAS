"""
Kotak Bank Transaction Report Generator
=======================================
...
# Initialize the unified Kotak classifier (singleton)
_classifier = None
_learning_store = None

def get_classifier() -> KotakClassifier:
    """Get or create the unified Kotak classifier instance."""
    global _classifier
    if _classifier is None:
        _classifier = KotakClassifier()
        logger.info("Kotak classifier initialized: %s", _classifier.get_category_stats())
    return _classifier
...
def classify(row) -> Tuple[str, int]:
    ...
    learned = _learning_store.lookup(description, bank_name="Kotak")
    if learned:
        return {
    ...
    try:
        ...
        logger.debug("Kotak learning-store update skipped", exc_info=True)
    return result
...
def generate_report(
    ...
    """
    Generate 5-sheet Kotak transaction report.
    ...
    bank_name = (user_info or {}).get("bank_name", "Kotak Bank")
